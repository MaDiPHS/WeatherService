...
public class OpenMeteoAdapter {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(OpenMeteoAdapter.class);

    private final AmalgamationBean amalgamationBean;
    // Parameter mapping
    private final Map<Integer, String> ipmToOpenMeteoRequestHourly = Map.ofEntries(entry(1001, "temperature_2m"), // Only hourly
    ...
    private final Map<Integer, String> ipmToOpenMeteoRequestDaily = Map.ofEntries(entry(1003, "temperature_2m_min"), // Only daily
    ...
    private final Map<Integer, OpenMeteoParameter> ipmToOpenMeteoResponseHourly = Map.ofEntries(entry(1001, new OpenMeteoParameter(Variable.temperature, 2)), entry(1901, new OpenMeteoParameter(Variable.dew_point, 2)), entry(2001, new OpenMeteoParameter(Variable.precipitation)), entry(3001, new OpenMeteoParameter(Variable.relative_humidity, 2)), entry(4012, new OpenMeteoParameter(Variable.wind_speed, 10)), entry(5001, new OpenMeteoParameter(Variable.shortwave_radiation))

    );
    // Historic data goes up until 2 days before today. So e.g. on the day 2024-01-18, data is provided until UTC 2024-01-16 23:00
    private final Map<Integer, OpenMeteoParameter> ipmToOpenMeteoResponseDaily = Map.ofEntries(entry(1003, new OpenMeteoParameter(Variable.temperature, 2, Aggregation.minimum)), entry(1004, new OpenMeteoParameter(Variable.temperature, 2, Aggregation.maximum)), entry(2001, new OpenMeteoParameter(Variable.precipitation)), entry(5001, new OpenMeteoParameter(Variable.shortwave_radiation)));
    ...
    private final String HISTORIC_ENDPOINT_TPL = "https://archive-api.open-meteo.com/v1/archive?format=flatbuffers&latitude=%1$s&longitude=%2$s&start_date=%3$s&end_date=%4$s&%5$s&timezone=%6$s";
    private final String FORECAST_ENDPOINT_TPL = "https://api.open-meteo.com/v1/forecast?format=flatbuffers&past_days=2&latitude=%1$s&longitude=%2$s&%3$s&timezone=%4$s";
    ...
    public String getParametersURLString(List<Integer> parameters, Integer interval) {
        if (interval.equals(WeatherDataUtil.INTERVAL_HOURLY)) {
            return "hourly=" + String.join(",", parameters.stream().filter(p -> this.ipmToOpenMeteoRequestHourly.containsKey(p)).map(p -> this.ipmToOpenMeteoRequestHourly.get(p)).collect(Collectors.toList()));
        }
        ...
        if (interval.equals(WeatherDataUtil.INTERVAL_DAILY)) {
            ...
            String dailyStr = "daily=" + String.join(",", parameters.stream().filter(p -> this.ipmToOpenMeteoRequestDaily.containsKey(p)).map(p -> this.ipmToOpenMeteoRequestDaily.get(p)).collect(Collectors.toList()));
            // Then add hourly ones that daily doesn't contain - that need to be aggregated after retrieval
            String hourlyStr = "hourly=" + String.join(",", parameters.stream().filter(p -> !this.ipmToOpenMeteoRequestDaily.containsKey(p)).map(p -> this.ipmToOpenMeteoRequestHourly.get(p)).collect(Collectors.toList()));
    ...
    public WeatherData getData(Double longitude, Double latitude, ZoneId tzForLocation, Instant timeStart, Instant timeEnd, Integer interval, List<Integer> parameters) {
        ...
        parameters = new ArrayList(parameters.stream().filter(p -> this.ipmToOpenMeteoRequestDaily.containsKey(p) || this.ipmToOpenMeteoRequestHourly.containsKey(p)).collect(Collectors.toList()));
        ...
        encodedTimeZone = URLEncoder.encode(tzForLocation.toString(), StandardCharsets.UTF_8);
        ...
        String historicURL = needHistoric ? String.format(this.HISTORIC_ENDPOINT_TPL, latitude, longitude, timeStart.atZone(tzForLocation).toLocalDate().format(DateTimeFormatter.ISO_LOCAL_DATE), needForecasts ? Instant.now().atZone(tzForLocation).toLocalDate().atStartOfDay(tzForLocation).format(DateTimeFormatter.ISO_LOCAL_DATE) : timeEnd.atZone(tzForLocation).toLocalDate().atStartOfDay(tzForLocation).format(DateTimeFormatter.ISO_LOCAL_DATE), this.getParametersURLString(parameters, interval), encodedTimeZone) : null;
        LOGGER.debug(historicURL);
        String forecastURL = needForecasts ? String.format(this.FORECAST_ENDPOINT_TPL, latitude, longitude, this.getParametersURLString(parameters, interval), encodedTimeZone) : null;
        ...
        try {
            ...
            */
            ...
            // using forecast data
            List<WeatherData> weatherData = historicData != null && forecastData != null ? List.of(forecastData, historicData) : historicData != null ? List.of(historicData, new WeatherData()) : List.of(forecastData, new WeatherData());
    ...
    private WeatherApiResponse getWeatherApiResponse(String url) throws IOException {
        ...
        if (response.isSuccessful()) {
            ...
            WeatherApiResponse mApiResponse = WeatherApiResponse.getRootAsWeatherApiResponse(buffer.position(4));
            buffer.clear();
            return mApiResponse;
        }
